...
export default class AuthController {
        ...
        try {
            ...
            const graphResponse = await getGraphClient(accessToken)
                ...
                .select(
                    ...
                    'displayName,mail,businessPhones,id'
                )
    ...
    public async signIn(req: AuthenticatedRequest, res: Response) {
        const tokenValue = req.headers['authorization'] ? req.headers['authorization'].split(' ')[1] : '';
        ...
        const user = {
            ...
            name: (graphData as GraphData).name,
            email: (graphData as GraphData).email,
            phone: (graphData as GraphData).phone,
